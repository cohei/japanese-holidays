...
data Holiday
  ...
  | PublicHoliday
  | SubstituteHoliday
  | CeremonyOfTheEnthronement
  | RitesOfShowaEmperorFuneral
  | CeremonyOfPrinceAkihitoMarriage
  | CeremonyOfPrinceNaruhitoMarriage
  deriving (Eq, Show)
...
toJapanese HealthSportsDay                  = "体育の日"
toJapanese CultureDay                       = "文化の日"
toJapanese LabourThanksgivingDay            = "勤労感謝の日"
toJapanese Emperor'sBirthday                = "天皇誕生日"
toJapanese PublicHoliday                    = "国民の休日"
toJapanese SubstituteHoliday                = "振替休日"
toJapanese CeremonyOfTheEnthronement        = "即位礼正殿の儀"
toJapanese RitesOfShowaEmperorFuneral       = "昭和天皇の大喪の礼"
toJapanese CeremonyOfPrinceAkihitoMarriage  = "皇太子明仁親王の結婚の儀"
toJapanese CeremonyOfPrinceNaruhitoMarriage = "皇太子徳仁親王の結婚の儀"
...
holiday day =
  ...
  (1959, 4, 10) -> Just CeremonyOfPrinceAkihitoMarriage
  (_, 5, 3) -> Just ConstitutionMemorialDay
  (y, 5, 4)
    | y >= 2007 -> Just GreeneryDay
    | y >= 1986 && isMonday day -> Just PublicHoliday
  (_, 5, 5) -> Just Children'sDay
  (y, 5, 6)
    | y >= 2007 && (isTuesday day || isWednesday day) -> Just SubstituteHoliday
  (1993, 6, 9) -> Just CeremonyOfPrinceNaruhitoMarriage
  (y, 7, _)
                  ...
                  then Just AutumnalEquinoxDay
                       ...
                       then if weekOfMonth == 2 && isMonday day
                            then Just RespectForTheAgedDay
                                 ...
                                 then Just PublicHoliday
                                 else Nothing
                       else if y >= 1966 && d == 15
                            then Just RespectForTheAgedDay
                            else Nothing
  (y, 10, _)
    | y >= 2000 -> if weekOfMonth == 1 && isMonday day then Just HealthSportsDay else Nothing
  (y, 10, 10)
    | y >= 1966 -> Just HealthSportsDay
  (_, 11, 3) -> Just CultureDay
  (_, 11, 23) -> Just LabourThanksgivingDay
  (1990, 11, 12) -> Just CeremonyOfTheEnthronement
  (y, 12, 23)
    | y >= 1989 -> Just Emperor'sBirthday
  _ | isMonday day && isHoliday (addDays (-1) day) -> Just SubstituteHoliday
  _ -> Nothing
