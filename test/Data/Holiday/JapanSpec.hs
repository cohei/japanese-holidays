...
spec = parallel $ do
  describe "holiday" $ do
    ...
    holidays `for_` \(year, month, day, holiday') ->
      ...
      in
        it (concat ["returns ", show holiday', " for ", date]) $ do
          holiday (fromGregorian year month day) `shouldBe` Just holiday'

    it "returns Nothing for 2015-09-15" $ do
    ...
    describe "New Emperor's Birthday" $ do
      ...
      it "2019-02-23 is not Emperor's Birthday" $ do
        holiday (fromGregorian 2019 2 23) `shouldBe` Nothing
      it "2020-02-23 is Emperor's Birthday" $ do
